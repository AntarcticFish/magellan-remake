...
AddReplicableComponent('event_trigger_magellan_remake')

AddPlayerPostInit(function (inst)
    inst:ListenForEvent('event_trigger_magellan_remake_triggered',function ()
        if inst == ThePlayer and inst.replica.event_trigger_magellan_remake then
            local event_name = inst.replica.event_trigger_magellan_remake:GetEventName() or ''
            if event_name == '' then
                return
            end
            local type = inst.replica.event_trigger_magellan_remake:GetType() or ''
            if type == '' then
                return
            end
    ...
    inst:AddComponent('event_trigger_magellan_remake')
end)
...
function EntityScript:PushEvent(event,data,...)
    ...
    if allevents[event] then
        -- 筛选player
        if self and self.components.event_trigger_magellan_remake then
            local type -- 被哪个type拦了
            -- hook
            local allow = true
            local equips = SUGAR_magellan_remake:getAllEquipments(self)
            ...
            if not allow then
                ...
                if type then
                    -- 传递数据
                    self.components.event_trigger_magellan_remake:Push(event,type)
                end
                return
            end
        end
...
function State:HandleEvent(sg,eventame,data,...)
    ...
    if allevents[sg] then
        if inst and inst.components.event_trigger_magellan_remake then
            local allow = true
            local equips = SUGAR_magellan_remake:getAllEquipments(inst)
