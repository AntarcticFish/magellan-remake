...
function (self)
    if self.inst.components.magellan_remake_dmg_modifier == nil then
       self.inst:AddComponent('magellan_remake_dmg_modifier')
    end
    ...
    function self:GetAttacked(attacker,damage,weapon,stimuli,spdamage,...)
        ...
        if allow_to_continue then
            if attacker then
                if attacker.components.magellan_remake_dmg_modifier then
                    -- 先执行回调函数
                    attacker.components.magellan_remake_dmg_modifier:_RunOnHitAlways(victim)
                    -- 允许物理伤害加算加成
                    local physical_add = attacker.components.magellan_remake_dmg_modifier.physical:Get()
                    damage = damage + physical_add
                    -- 允许特殊伤害加算加成
                    for k,v in pairs(attacker.components.magellan_remake_dmg_modifier.spdamage) do
                        local spd_add = v:Get()
                        spdamage[k] = (spdamage[k] or 0) + spd_add
                    end
                    -- 允许物理伤害乘算加成
                    local physical_mult = attacker.components.magellan_remake_dmg_modifier.mult_physical:Get()
                    damage = damage * physical_mult
                    -- 允许特殊伤害乘算加成
                    for k,v in pairs(attacker.components.magellan_remake_dmg_modifier.mult_spdamage) do
                        local spd_mult = v:Get()
                        spdamage[k] = (spdamage[k] or 0) * spd_mult
                    end

                    -- 暴击最后处理
                    if math.random() < attacker.components.magellan_remake_dmg_modifier:_GetCriticalChanceWithModifier() then
                        local dmgmult = attacker.components.magellan_remake_dmg_modifier:_GetCriticalDamageWithModifier()
                        damage = damage * dmgmult
                        for k,_ in pairs(spdamage) do
                            spdamage[k] = (spdamage[k] or 0) * dmgmult
                        end
                        -- 执行暴击回调函数
                        attacker.components.magellan_remake_dmg_modifier:_RunOnCriticalHit(victim)
                    end
                end
            end
        end
