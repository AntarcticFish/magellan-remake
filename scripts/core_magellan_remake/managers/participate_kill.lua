...
-- 参与击杀的人会推 `killed_magellan_remake_participate` 事件, 亲手击杀的还是监听 `killed`

---@class ent
---@field _magellan_remake_participate_kill_tbl table<ent, boolean> # 联合击杀贡献表

---@class event_data_killed_magellan_remake_participate
---@field victim ent
---@field attacker ent
...
function (self)
    ...
    function self:GetAttacked(attacker,damage,weapon,stimuli,spdamage,...)
        ...
        if attacker and attacker:IsValid() then
            if victim._magellan_remake_participate_kill_tbl == nil then
...
function(self)
    ...
    function self:SetVal(val,cause,afflicter,...)
        ...
        local victim = self.inst
        if self:IsDead() then
            if victim and victim._magellan_remake_participate_kill_tbl then
                for attacker,_ in pairs(victim._magellan_remake_participate_kill_tbl) do
                    if attacker and attacker:IsValid() then
                        if afflicter and afflicter ~= attacker then
                            attacker:PushEvent('killed_magellan_remake_participate', { victim = self.inst, attacker = attacker })
                        end
                    end
                end
            end
