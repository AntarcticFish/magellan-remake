...
---@class components
---@field magellan_remake_dmg_modifier component_magellan_remake_dmg_modifier

---@class component_magellan_remake_dmg_modifier
---@field inst ent
...
local magellan_remake_dmg_modifier = Class(
---@param self component_magellan_remake_dmg_modifier
---@param inst ent
function(self, inst)
    self.inst = inst
    -- self.val = 0
...
-- function magellan_remake_dmg_modifier:OnSave()
--     return {
--         -- val = self.val
--     }
-- end

-- function magellan_remake_dmg_modifier:OnLoad(data)
...
---@param m number
---@param key string
function magellan_remake_dmg_modifier:ModifierCritical(attri,modifier_type,source,m,key)
...
---@param source ent|string # 来源: 如果是实体,那么实体被移除时, 该修饰也会被移除
---@param key string
function magellan_remake_dmg_modifier:RemoveModifierCritical(attri,modifier_type,source,key)
    if attri == 'CriticalChance' then
        if modifier_type == 'add' then
...
---@param m number
---@param key string
function magellan_remake_dmg_modifier:ModifierNormal(dmg_type,modifier_type,source,m,key)
    if modifier_type == 'add' then
...
---@param source ent|string # 来源: 如果是实体,那么实体被移除时, 该修饰也会被移除
---@param key string
function magellan_remake_dmg_modifier:RemoveModifierNormal(dmg_type,modifier_type,source,key)
    if modifier_type == 'add' then
        if dmg_type == 'physical' then
...
---@return number
---@nodiscard
function magellan_remake_dmg_modifier:_GetCriticalChanceWithModifier()
...
---@return number
---@nodiscard
function magellan_remake_dmg_modifier:_GetCriticalDamageWithModifier()
...
function magellan_remake_dmg_modifier:SetOnCriticalHit(id,fn)
    self.on_critical_hit_fn[id] = fn
end

---移除暴击触发函数
---@param id string
function magellan_remake_dmg_modifier:RemoveOnCriticalHit(id)
    self.on_critical_hit_fn[id] = nil
end

---运行所有暴击触发函数
---@param victim ent
function magellan_remake_dmg_modifier:_RunOnCriticalHit(victim)
...
---@param id string
---@param fn fun(victim:ent)
function magellan_remake_dmg_modifier:SetOnHitAlways(id,fn)
    self.on_hit_fn_always[id] = fn
end

---移除 无论如何都会再击中时触发 的函数
---@param id string
function magellan_remake_dmg_modifier:RemoveOnHitAlways(id)
    self.on_hit_fn_always[id] = nil
end

---运行所有 无论如何都会再击中时触发 函数
---@param victim ent
function magellan_remake_dmg_modifier:_RunOnHitAlways(victim)
    for _,v in pairs(self.on_hit_fn_always) do
        v(victim)
    end
end

return magellan_remake_dmg_modifier