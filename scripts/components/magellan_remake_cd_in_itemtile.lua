...
---@class components
---@field magellan_remake_cd_in_itemtile component_magellan_remake_cd_in_itemtile

---comment
---@param self replica_magellan_remake_cd_in_itemtile
---@param value any
local function on_cur_cd(self, value)
    self.inst.replica.magellan_remake_cd_in_itemtile:SetCurCD(value)
end
...
local function on__show_itemtile(self, value)
    self.inst.replica.magellan_remake_cd_in_itemtile._show_itemtile:set(value)
end

---@class component_magellan_remake_cd_in_itemtile
---@field inst ent
...
local magellan_remake_cd_in_itemtile = Class(
---@param self component_magellan_remake_cd_in_itemtile
---@param inst ent
function(self, inst)
    self.inst = inst
    self.cd = 10
...
function magellan_remake_cd_in_itemtile:OnSave()
    return {
        cd = self.cd,
        cur_cd = self.cur_cd,
    }
...
function magellan_remake_cd_in_itemtile:OnLoad(data)
...
function magellan_remake_cd_in_itemtile:Init(cd,shown_on_itemtile)
    self.cd = cd
...
---更改默认cd
---@param cd integer
function magellan_remake_cd_in_itemtile:ChangeDefaultCD(cd)
    self.cd = cd
end
...
---@protected
function magellan_remake_cd_in_itemtile:_StartCD(cd,start_without_onstartcdfn)
    -- 更新默认cd
    if cd then
        self.cd = cd
    end
...
---开始cd
---@param cd integer|nil # 不填用默认cd,填了则会更新默认cd
function magellan_remake_cd_in_itemtile:StartCD(cd)
...
---@return boolean
---@nodiscard
function magellan_remake_cd_in_itemtile:IsCD()
    return self.cur_cd > 0
end

---当在cd中时修改剩余cd
---@param val integer
function magellan_remake_cd_in_itemtile:SetCurCD(val)
    if self:IsCD() then
        self.cur_cd = val
    end
end
...
function magellan_remake_cd_in_itemtile:SetOnStartCD(fn)
    self.onstartcdfn = fn
end

---设置cd转好时调用的函数
---@param fn fun(this:ent,...):...
function magellan_remake_cd_in_itemtile:SetOnFinishCD(fn)
    self.onfinishcdfn = fn
end

---是否显示在itemtile上,不设置默认显示
---@param shown boolean
function magellan_remake_cd_in_itemtile:ShowItemTile(shown)
    self._show_itemtile = shown
end

return magellan_remake_cd_in_itemtile